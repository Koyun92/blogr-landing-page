...
const dropdownList = () => {
  const listItem = document.querySelectorAll(".nav-dropdown-button");
  listItem.forEach((item) => {
    item.addEventListener("click", () => {
      item.nextElementSibling.classList.toggle("nav-dropdown-list--active");
      item.lastChild.classList.toggle("nav-icon--rotate");
    });
  });
};
